...
def send_path_mission(points: List[Point]) -> None:
    ...
    if send_command(payload):
        ...
        st.session_state.last_mission_send_status = "Mission was not sent. Connect serial first."
...
with mission_tab:
    ...
    with left_col:
        ...
        if not is_serial_connected():
            st.warning("Rover is not connected. Send Path Mission will auto-connect to COM8 first.")
